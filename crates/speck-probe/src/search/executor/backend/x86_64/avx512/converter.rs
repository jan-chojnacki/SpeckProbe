...
mod tests {
    ...
    fn avx512_u16x2_broadcast(#[case] input: [u16; 2], #[case] expected: [u16; 2]) {
        if !is_x86_feature_detected!("avx512bw") {
            return;
        }
        unsafe {
            let [va, vb] = avx512_u16x2_block_to_vec(input);
            let a: [u16; 32] = std::mem::transmute(va);
            let b: [u16; 32] = std::mem::transmute(vb);
    ...
    fn avx512_u32x2_broadcast(#[case] input: [u32; 2], #[case] expected: [u32; 2]) {
        if !is_x86_feature_detected!("avx512f") {
            return;
        }
        unsafe {
            let [va, vb] = avx512_u32x2_block_to_vec(input);
            let a: [u32; 16] = std::mem::transmute(va);
            let b: [u32; 16] = std::mem::transmute(vb);
            assert!(a.iter().all(|&x| x == expected[0]));
            assert!(b.iter().all(|&x| x == expected[1]));
        }
    }
    ...
    fn avx512_u64x2_broadcast(#[case] input: [u64; 2], #[case] expected: [u64; 2]) {
        if !is_x86_feature_detected!("avx512f") {
            return;
        }
        unsafe {
            let [va, vb] = avx512_u64x2_block_to_vec(input);
            let a: [u64; 8] = std::mem::transmute(va);
            let b: [u64; 8] = std::mem::transmute(vb);
            assert!(a.iter().all(|&x| x == expected[0]));
            assert!(b.iter().all(|&x| x == expected[1]));
        }
    }
