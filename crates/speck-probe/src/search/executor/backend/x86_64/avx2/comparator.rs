...
mod test {
    use super::*;
    use crate::search::executor::backend::x86_64::avx2::{
    ...
    fn avx2_u16_hit(#[case] expected_data: [u16; 2]) {
        if !is_x86_feature_detected!("avx2") {
            return;
        }
        unsafe {
            let expected = avx2_u16x2_block_to_vec(expected_data);
            let key = AVX2Key::<16, 8, 0>::new(&[], [V_U16; 16], SpeckVersion::Speck32_64);
            let mut out = Vec::new();
            avx2_block_compare_u16(&expected, &expected, &key, &mut out);
    ...
    fn avx2_u16_miss(#[case] expected_data: [u16; 2], #[case] value_data: [u16; 2]) {
        if !is_x86_feature_detected!("avx2") {
            return;
        }
        unsafe {
            let expected = avx2_u16x2_block_to_vec(expected_data);
            let v = avx2_u16x2_block_to_vec(value_data);
            let key = AVX2Key::<16, 8, 0>::new(&[], [V_U16; 16], SpeckVersion::Speck32_64);
            let mut out = Vec::new();
            avx2_block_compare_u16(&expected, &v, &key, &mut out);
            assert!(out.is_empty());
        }
    }
    ...
    fn avx2_u32_hit(#[case] expected_data: [u32; 2]) {
        if !is_x86_feature_detected!("avx2") {
            return;
        }
        unsafe {
            let expected = avx2_u32x2_block_to_vec(expected_data);
            let key = AVX2Key::<8, 12, 4>::new(PREFIX_U32, [V_U32; 8], SpeckVersion::Speck64_96);
            let mut out = Vec::new();
            avx2_block_compare_u32(&expected, &expected, &key, &mut out);
            assert_eq!(out.len(), 8);
            for k in &out {
    ...
    fn avx2_u32_miss(#[case] expected_data: [u32; 2], #[case] value_data: [u32; 2]) {
        if !is_x86_feature_detected!("avx2") {
            return;
        }
        unsafe {
            let expected = avx2_u32x2_block_to_vec(expected_data);
            let v = avx2_u32x2_block_to_vec(value_data);
            let key = AVX2Key::<8, 12, 4>::new(PREFIX_U32, [V_U32; 8], SpeckVersion::Speck64_96);
            let mut out = Vec::new();
            avx2_block_compare_u32(&expected, &v, &key, &mut out);
            assert!(out.is_empty());
        }
    }
    ...
    fn avx2_u64_hit(#[case] expected_data: [u64; 2]) {
        if !is_x86_feature_detected!("avx2") {
            return;
        }
        unsafe {
            let expected = avx2_u64x2_block_to_vec(expected_data);
            let key = AVX2Key::<4, 16, 8>::new(PREFIX_U64, [V_U64; 4], SpeckVersion::Speck128_128);
            let mut out = Vec::new();
            avx2_block_compare_u64(&expected, &expected, &key, &mut out);
            assert_eq!(out.len(), 4);
            for k in &out {
    ...
    fn avx2_u64_miss(#[case] expected_data: [u64; 2], #[case] value_data: [u64; 2]) {
        if !is_x86_feature_detected!("avx2") {
            return;
        }
        unsafe {
            let expected = avx2_u64x2_block_to_vec(expected_data);
            let v = avx2_u64x2_block_to_vec(value_data);
            let key = AVX2Key::<4, 16, 8>::new(PREFIX_U64, [V_U64; 4], SpeckVersion::Speck128_128);
            let mut out = Vec::new();
            avx2_block_compare_u64(&expected, &v, &key, &mut out);
            assert!(out.is_empty());
        }
    }
}
