...
cargo rustc --bin stack_measure -- --emit=llvm-ir -C opt-level=0 -C debuginfo=0 -C codegen-units=1 2>/dev/null
cp target/debug/deps/stack_measure-*.ll /tmp/bin_o0.ll
...
cargo rustc --bin stack_measure -- --emit=llvm-ir -C opt-level=3 -C debuginfo=0 -C codegen-units=1 2>/dev/null
cp target/debug/deps/stack_measure-*.ll /tmp/bin_o3.ll
